...
/** Real questions the grounded system prompt (app/api/chat/route.ts, "ar" locale) can actually answer well. */
const QUICK_ACTIONS = [
  { icon: Layers, label: "ماذا تتضمن كل باقة؟" },
  { icon: Network, label: "كيف يتكامل مع مايكروسوفت؟" },
  { icon: Building2, label: "هل يناسب الشركات العقارية؟" },
  { icon: Lock, label: "كيف تعمل الصلاحيات حسب الدور؟" },
  { icon: Calculator, label: "كم تكلفة الباقة B؟" },
  { icon: AlertTriangle, label: "قطاعي غير مدرج، هل تبنون باقات مخصصة؟" },
];
...
export default function RuixenMoonChatAR() {
  ...
  const send = async (text: string) => {
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ message: trimmed, locale: "ar" }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "تعذّر الوصول إلى المساعد.");
      setReply(data.reply);
    } catch (err) {
      setError(err instanceof Error ? err.message : "تعذّر الوصول إلى المساعد.");
    } finally {
      setThinking(false);
    }
  };
  ...
  return (
    <div className="relative flex w-full flex-col items-center overflow-hidden rounded-3xl border border-slate-200 bg-white px-6 py-20 shadow-sm">
      ...
      <div className="flex flex-col items-center text-center">
        <span className="mb-4 inline-flex items-center gap-1.5 rounded-full border border-cyan-100 bg-cyan-50 px-3.5 py-1.5 text-xs font-medium text-cyan-700">
          <Sparkles size={13} /> مدعوم بذكاء اصطناعي حقيقي
        </span>
        <h2 className="text-3xl font-semibold tracking-tight text-slate-900 sm:text-4xl">اسأل Collectionat</h2>
        <p className="mt-2 max-w-md text-slate-600">
          مساعد بذكاء اصطناعي حقيقي يعرف باقات Collectionat وتكاملاته وقطاعاته — اسأله عمّا تشاء.
        </p>
      </div>
        ...
        <div className="w-full max-w-2xl">
          <div className="relative rounded-xl border border-slate-200 bg-slate-50">
            <Textarea
              ...
              placeholder="ما الفرق بين الباقة A والباقة B؟"
              className={cn(
